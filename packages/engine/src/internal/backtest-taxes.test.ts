...
describe("computeMonthlyTax", () => {
  it("exempts a month whose stock sales stay at or under the exemption, gains only", () => {
    const tax = computeMonthlyTax("2024-01", centavos(20_000_00), centavos(1_000_00), costModel);
    expect(tax).toEqual({
      month: "2024-01",
  ...
  it("drops the whole stock result (even a loss) in an exempt month", () => {
    const tax = computeMonthlyTax("2024-01", centavos(5_000_00), centavos(-200_00), costModel);
  ...
  it("taxes the whole stock gain at 15% when sales exceed the exemption", () => {
    const tax = computeMonthlyTax("2024-02", centavos(21_000_00), centavos(10_000_00), costModel);
    expect(tax.exemptGain).toBe(centavos(0));
