...
- **Integration**: Route Handlers + Drizzle against a fresh Postgres service container behind Neon's
  local proxy in CI (ADR-0016), or `fetha-preview` locally, including provider adapters with
  recorded fixtures.
...
- Vercel skips preview builds when only docs, ADRs, agent files or CI workflows change
  (`ignoreCommand` in `apps/web/vercel.json` → `apps/web/scripts/vercel-ignore-build.sh`).
