...
describe("evaluateSignalsForSession", () => {
  ...
  it("counts a user whose active strategies are already caught up separately from one actually evaluated, and lets `force` redo it anyway (round 3 item 3)", async () => {
    const db = getDb();
    const session = randomSession();
    createdSessions.push(session);
    const ticker = randomTicker();
    ...
    // A corrected candle for the same session: without `force`, the
    // watermark would still short-circuit the whole strategy loop.
    await insertCandle(ticker, session, "12.340000");
    const third = await evaluateSignalsForSession(db, [session], { force: true });
    expect(third.errors).toEqual([]);
    expect(third.usersEvaluated).toBe(1);
    expect(third.usersAlreadyCaughtUp).toBe(0);
