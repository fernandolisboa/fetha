...
- Better Auth's tables (`user`, `session`, `account`, `verification`) are generated by its CLI
  (`pnpm --filter @fetha/web db:auth-schema`, pinned via the `@better-auth/cli` devDependency and
  invoked through `pnpm exec`, never a bare `npx`) into `src/db/schema/auth.ts` and migrated with
